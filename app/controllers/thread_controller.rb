class ThreadController < ApplicationController
  def show
    ...
    @t_direct_threads = TDirectThread.select("t_direct_threads.id as id, m_users.name, t_direct_threads.directthreadmsg, t_direct_threads.t_direct_message_id, t_direct_threads.created_at,
                                           ...
                                           .order(id: :asc)
    @t_group_messages = TGroupMessage.select("distinct m_users.name, t_group_messages.groupmsg, t_group_messages.id as id, t_group_threads.t_group_message_id, t_group_messages.m_channel_id as channel_id, m_channels.channel_status as channel_status,
                                           ...
                                           .order(created_at: :asc)
    @t_group_threads = TGroupThread.select("m_users.name, m_channels.channel_name, t_group_threads.groupthreadmsg, t_group_threads.id, t_group_threads.t_group_message_id as t_group_message_id, t_group_threads.created_at, ARRAY_AGG(t_group_thread_msg_files.file) as file_urls")
